    ...
    <>
      ...
      <Form className="pf-v6-u-mt-md">
        ...
        <FormGroup label="Subnet CIDR" isRequired fieldId="udn-cidr">
          ...
          <FormHelperText>
            Dual-stack clusters may set 2 subnets (one for each IP family), otherwise only 1 subnet is allowed. The format
            should match standard CIDR notation (for example, &apos;192.168.123.0/24&apos;).
          </FormHelperText>
        </FormGroup>
      </Form>
      <CreateFormActions canCreate={canCreate} onCancel={onCancel} onCreate={handleCreate} />
...
export function CreateNncpForm({ onCancel, onCreated }: CreateFormProps<NncpRecord>) {
  ...
  return (
    <>
      <Content component="p">
        Configure node network interfaces and bridges on selected nodes. This prototype creates a policy that provisions{" "}
        <strong>br-localnet</strong> on worker nodes.
      </Content>
      <Form className="pf-v6-u-mt-md">
